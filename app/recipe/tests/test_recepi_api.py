...
class PrivateRecipeApiTests(TestCase):
    ...
    def setUp(self):
        self.client = APIClient()
        self.user = creat_user(email='test@example.com', password='test123')
    ...
    def test_delete_recipe(self):
        ...
        self.assertAlmostEqual(res.status_code, status.HTTP_204_NO_CONTENT)
        self.assertFalse(Recipe.objects.filter(id=recipe.id).exists())


    def test_recipe_other_users_error(self):
        new_user = creat_user(email= '2G8Qg@example.com', password= 'test123')
    ...
    def test_create_revipe_with_existing_tags(self):
        """Test creating a recipe with existing tags."""
        tag = Tag.objects.create(user=self.user, name='Indian')
        payload = {
            'title': 'Thai www Curry',
            'time_minutes': 60,
            'price': Decimal('4.50'),
            'tags': [{'name': 'Indian'}, {'name':'breach'}],
        }
        res = self.client.post(RECIPES_URL, payload, format='json')
